 ...
 */
public class DocumentBasedTfIdf {
     ...
     */
    public double idfCalculator(List<APIDocMsg> allTerms, String termToCheck) {
        double count = 0;
        for (APIDocMsg APILibraryData : allTerms) {
        	String[] ss = APILibraryData.getMethodDescription().split(" ");
            for (String s : ss) {
                if (s.equalsIgnoreCase(termToCheck)) {
                    count++;
                    break;
